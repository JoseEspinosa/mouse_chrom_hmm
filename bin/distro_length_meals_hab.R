...
hist(x, breaks=seq(0,5500,by=10))
hist_bed <- hist(x, breaks=seq(0,5500,by=10), plot=FALSE)
hist_bed$counts
...
# hist_br_100$density
min_values <- hist_br_100$mids[ which (hist_br_100$mid > 3 & hist_br_100$mid < 4 & hist_br_100$counts ==5) ]

ggplot(data.frame_bed, aes(x=length_log)) +
  stat_density(aes(y=..count..), color="black", fill="blue", alpha=0.1) +
  scale_x_continuous(breaks=c(0:9), expand=c(0,0)) +
  scale_y_continuous(breaks=c(0,125,250,375,500,625,750,875,1000,1125,1250,1375), expand=c(0,0)) +
  theme_bw() + geom_vline(xintercept=min_values[2], linetype="dashed")
