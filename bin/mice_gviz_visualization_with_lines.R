...
l_gr_data_tr_bg <- lapply (seq_along(l_granges_bg), function (i_group_exp) {
  lapply (seq_along (l_granges_bg[[i_group_exp]]),  function (i_track) {                                                     
    ...
    d_track <- DataTrack(granges_obj,
                         ...
                         ylim=c(min_heatmap, max_heatmap),
                         showAxis = F, name = id)
    return (d_track)
  })
})
...
## Generating plot summarizing mean bedGraph value of groups
...
color_by_tr <- unlist(l_gr_color[unique(group_lab)])
common_bedg_dt <- DataTrack(gr_common_intervals, name = "Mean intake (grams)", type = "a",
                                showSampleNames = TRUE,
                                groups = group_lab,
                                lwd.baseline = 12,
                                ylim=c(min_line_plot, max_line_plot),
                                col = color_by_tr,
                                background.title = col_back_title,
                                legend = FALSE)
...
p <- plotTracks(c(g_tr, unlist(l_gr_annotation_tr_bed), unlist(l_gr_data_tr_bg), common_bedg_dt, phases_tr, unlist(ctracks)),
